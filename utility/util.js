const Utils = {
    ...
    isArr: (what) => {
        return (
            !Utils.isNull(what) &&
            what.constructor.toString().indexOf('Array') > -1
        )
    },
    ...
    isLink: (what) => {
        return (
            Utils.isStr(what) &&
            /^((https|http|ftp|rtsp|mms)?:\/\/)[^\s]+/.test(what)
        )
    },
    ...
    numberFormat: (val, decimals, thousandsSep) => {
        val = (
            typeof decimals !== 'undefined' ? val.toFixed(decimals) : val
        ).toString()
        // 分离数字的小数部分和整数部分
        var parts = val.split('.')
        ...
        parts[0] = parts[0]
            .toString()
            .replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1' + (thousandsSep || ','))
